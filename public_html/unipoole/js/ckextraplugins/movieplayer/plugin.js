...
Movie.prototype.getInnerHTML = function (objectId){
	...
	if(mimeSupported.contains(this.contentType)) {
		...
		if(this.contentType == "video/quicktime") {
			// QUICKTIME
			var objectheight=this.height + 16 // controller add 16px to height
			...
			s += '  <param name="src" value="' + encodeURI(this.url) +'"/> ';
			s += '  <param name="scale" value="aspect" /> ';
			s += '  <param name="controller" value="true" /> '; //add 16px to height*
			s += '  <param name="autoplay" value="false" /> ';
			s += '  <embed ';
			s += '       type="'+this.contentType+'" ';
  		    s += '       pluginspage="http://www.apple.com/quicktime/download/" ';
			s += '       width="'+this.width+'" height="'+objectheight+'" ';
			s += '       src = "'+ encodeURI(this.url) +'" ';
			s += '       scale="aspect" ';
            s += '       controller="true"';  //*the controller adds 16px to height* 
			s += '       autoplay="false" /> ';
            s += '</object>';
			
			
			
		}else if(this.contentType == "audio/x-m4a") {
			//QUICKTIME AUDIO
			s += '<OBJECT id="movie' + rnd + '" ';
			s += '        classid="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" ';
			s += '        codebase="http://www.apple.com/qtactivex/qtplugin.cab" '
			s += '        width="320" height="16" >';
		    s += '  <PARAM name="src" value="'+ encodeURI(this.url) +'" />';
			s += '  <PARAM name="autoplay" value="'+this.autoplay+'" />';
			s += '  <PARAM name="controller" value="true" />';
			s += '  <OBJECT type="'+this.contentType+'" ';
    ...
    CKEDITOR.dialog.add( 'movie.dlg', function( api ) {
            var dialogDef = {
                ...
                onOk : function() {
					...
					var ext = getExtension(tab1doc.getElementById('txtUrl').value);
					if (ext=="m4a" || ext =="m4v"){
						var dialog = this,
									link = editor.document.createElement( 'a' );
					
						link.setAttribute('href',tab1doc.getElementById('txtUrl').value);
						link.setAttribute('taget','_blank');
						if (ext == "m4v"){
							link.setHtml("Right click here and choose Save to download the movie");
						} else {
								link.setHtml("Right click here and choose Save to download the audio file");
						}
						this._.editor.insertHtml("<br>");
						this._.editor.insertElement(link);
					}
...
function getExtension(url) {
    var ext = url.match(/\.(avi|asf|fla|flv|mov|mp3|mp4|mpg|mpeg|qt|swf|wma|wmv|m4v|m4a)$/i);
...
function getContentType(url) {
    ...
    var contentType =
            ...
            (ext=="mov"||ext=="qt"||ext=="m4v") ? "video/quicktime":
			(ext=="m4a") ? "audio/x-m4a": "video/x-ms-wmv";
    return contentType;
}
